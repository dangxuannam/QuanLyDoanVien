# Hệ Thống Quản Lý Đoàn Viên (QuanLyDauTu)
...
C:\QuanLyDauTu\
│
├── QuanLyDauTu.Web/       # Backend - Chứa mã nguồn ASP.NET Web API, Controllers, Services, Models.
├── QuanLyDauTu.Angular/   # Frontend - Chứa source code giao diện trên nền tảng Angular.
├── Database/              # Script SQL để khởi tạo Database và thiết lập Seed Data (Menu, Phân quyền).
└── QuanLyDauTu.sln        # Visual Studio Solution File cho Backend.
```
...
7.  **Quản lý Đoàn viên (DOAN_VIEN):** Phân hệ chuyên biệt giúp quản lý chi tiết thông tin đoàn viên, phân loại đoàn viên/đảng viên, với khả năng Import dữ liệu từ file Excel có cấu trúc Header phức tạp, bao gồm các trường Ghi chú Cấp ủy, Giới tính, Chức vụ.
...
2. Mở thư mục `Database` và chạy lần lượt các script:
   - `001_CreateDatabase.sql`: Tạo Database `QuanLyDauTu` và các bảng cơ sở.
...
1. Mở file `QuanLyDauTu.sln` bằng **Visual Studio 2022**.
2. Kiểm tra chuỗi kết nối (Connection String) trong file `QuanLyDauTu.Web\Web.config`:
   
   <add name="QuanLyDauTuContext" connectionString="Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=QuanLyDauTu;Integrated Security=True;..." providerName="System.Data.SqlClient" />
...
1. Mở Command Prompt (Terminal) tại thư mục Frontend:
   ```bash
   cd C:\QuanLyDauTu\QuanLyDauTu.Angular
   ```
